...
This document defines the TypeScript coding standards and best practices for this project. The project uses **Node.js 22+** with `--experimental-strip-types` as the runtime, which provides native TypeScript execution without a build step.
...
**ALWAYS include `.ts` file extensions in imports**

Node.js with `--experimental-strip-types` runs TypeScript directly, so always use `.ts` extensions that match the actual files on disk.
...
// ❌ Bad - .js extension when file is .ts
import { createUser } from './user-service.js';
```

**Rationale**:

- Node.js runs TypeScript directly with type stripping - use the actual file extension
- Explicit `.ts` extensions match the file on disk and avoid ambiguity
...
class Logger {
  ...
  #log = (level: string, message: string, meta: LogContext = {}): void => {
    ...
    console.log(this.#isJson ? JSON.stringify(entry) : `[${entry.timestamp}] ${level.toUpperCase()}: ${message} ${JSON.stringify({ ...this.#context, ...meta })}`);
  };
...
class Services {
  ...
  destroy = async (): Promise<void> => {
    await Promise.all(Array.from(this.#instances.values()).map(async (i) => 
...
**ALWAYS use Node's built-in test runner** (`node:test`) - it's fast and requires no additional dependencies.
...
import { createUser, getUserById, UserNotFoundError } from './user.ts';
...
Use `mock` from `node:test` for function mocking:
...
it('calls the callback with correct arguments', () => {
  const callback = mock.fn((x: number) => x * 2);

  callback(5);
  callback(10);

  assert.strictEqual(callback.mock.callCount(), 2);
  assert.deepStrictEqual(callback.mock.calls[0].arguments, [5]);
  assert.strictEqual(callback.mock.calls[0].result, 10);
});
```

### Module Mocking

Use `mock.module` to mock entire modules (Node.js 22+):

```typescript
import { it, mock } from 'node:test';
import assert from 'node:assert';

// Register mock before importing
await mock.module('./database.ts', {
  namedExports: {
    query: mock.fn(() => Promise.resolve([{ id: '1', name: 'Test' }])),
  },
});

// Now imports of ./database.ts will use the mock
...
Ensure strict mode and Node types are configured in `tsconfig.json`:
...
const exists = await fs.stat('./file.txt').then(() => true).catch(() => false);
```
...
Use Fastify for HTTP servers (recommended) or Node.js native:
